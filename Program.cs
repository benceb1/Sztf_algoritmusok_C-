    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            */
            ...
            int[] a = { 1,2,3,4};
            int[] b = { 5,6,7,8 };
            foreach (var item in HalmazokUnioja(a,a.Length,b,b.Length))
            {
        ...
        static bool ReszhalmazE(int[]a, int m, int[]b, int n)
        {
            int i = 0;
            int j = 0;
            while(i < m && j < n && a[i] >= b[j]) //pszeudoban i<=m és j<=n mert ott 1-el kezdődik
            {
                if (a[i] == b[j])
                    i += 1;
        ...
        //következőhöz tartozik
        static int[] PluszEgyMeretu(int[] forras)
        {
            int[] t = new int[forras.Length+1];
        ...
        //Halmazok uniója - (az algoritmus pszeudoja nem tükrözi a való életben lehetséges dolgokat) 
        
        static int[] HalmazokUnioja(int[] a1, int n1, int[] a2, int n2)
        {
            int[] b = new int[a1.Length + a2.Length];
            int i = 0; //pszeudoban 1
            int j = 0; //pszeudoban 1
            int db = -1; //pszeudoban 0

            n1 += 1; //itt nem kellenek
            n2 += 1;

            a1 = PluszEgyMeretu(a1); //pszeudoban a1[n1] <- +∞
            a2 = PluszEgyMeretu(a2);//pszeudoban a2[n2] <- +∞
